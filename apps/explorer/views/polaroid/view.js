/**
 * SimpleGallery 2026 - Explorer Polaroid View Plugin (apps/explorer/views/polaroid/view.js)
 */
(function(window) {
  'use strict';
  ...
  const PolaroidViewPlugin = {
    ...
    cssPath: 'apps/explorer/views/polaroid/view.css',
    ...
    renderItem(file, idx, ctx) {
      ...
      const mediaPreviewHtml = `<img src="${ctx.escapeHtml(file.thumb_url)}" alt="${ctx.escapeHtml(file.name)}" loading="lazy" draggable="false" />`;
