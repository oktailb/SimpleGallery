/**
 * SimpleGallery 2026 - Image Viewer Plugin
...
(function(window) {
  ...
  const ImageViewerPlugin = {
    ...
    open(file, options, ctx) {
      ...
      if (ctx.el.lightboxEditImageBtn) {
        ctx.el.lightboxEditImageBtn.style.display = isEditableImage ? 'inline-flex' : 'none';
        ctx.el.lightboxEditImageBtn.onclick = () => this.openImageEditor(file, ctx);
      }

      if (ctx.el.imageExplorerControls) {
        ctx.el.imageExplorerControls.style.display = 'flex';
      }
    ...
    bindExplorerEvents(ctx) {
      ...
      const zoomInBtn = document.getElementById('explorerZoomInBtn');
      const zoomOutBtn = document.getElementById('explorerZoomOutBtn');
      const zoomResetBtn = document.getElementById('explorerZoomResetBtn');
      const rotateBtn = document.getElementById('explorerRotateBtn');
