/**
 * SimpleGallery 2026 - Document Viewer Plugin
...
(function(window) {
  ...
  const DocViewerPlugin = {
    ...
    extensions: ['pdf', 'txt', 'md', 'json', 'csv', 'xml', 'html', 'js', 'css', 'php', 'py', 'sh', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'rtf', 'odt'],
    ...
    supportsPip: false,
    cssPath: 'css/viewers/doc.css',

    /**
     * Opens a document file in the Lightbox container with format-specific rendering
     * @param {Object} file
    ...
    async open(file, options, ctx) {
      ...
      else if (['txt', 'md', 'json', 'csv', 'xml', 'html', 'js', 'css', 'php', 'py', 'sh', 'log', 'ini', 'sql'].includes(ext)) {
        ctx.el.lightboxContent.innerHTML = `
          <div class="doc-viewer-modal-content">
